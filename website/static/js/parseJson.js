...
// config for single dataset Radar Chart
var config = {
    type: 'radar',
    data: {
...
// config for double dataset Radar Chart
var step = 10;
var max = 100;
var config_compare = {
    type: 'radar',
...
// config for dataset Radar Chart
var barchartData = {
...
//Function call to sent the httprequest to the appRoute to call
//the function in api.py to get the json response
function onGetPlayerStats() {
    event.preventDefault(); //prevent opening new pages
    var srchTerm = document.getElementById('srch-term-player').value;
    var url = '/playerStats/'+srchTerm;
    //instead send a http request to pass search term to appRoute in flask
    //to call the right api function to retrieve json data
    xmlHttpRequest = new XMLHttpRequest();
    xmlHttpRequest.open('get', url);
    xmlHttpRequest.onreadystatechange = function() {
        if (xmlHttpRequest.readyState == 4 && xmlHttpRequest.status == 200) {
            //if the http requset is sent successfully and a response is
            //given back; call the "playerStatsCallback" function to 
...
//Interpret the Json response and rerender the result div to display
//data and chart
function playerStatsCallback(jsonResponse) {
    ...
    if (statsList == -1) {
        ...
        //pass the datalist and playerName to be draw the chart
